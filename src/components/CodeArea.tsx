...
export default function CodeArea() {
  ...
  return (
    <div id="code-area" className="w-full h-full bg-light rounded-tl-xl rounded-br-[12px]">
      <div
        ref={scrollRef}
      ...
      >
        {opened.map((item) => {
          ...
          const active = selected === item ? "bg-[rgba(255,255,255,0.05)] text-gray-400" : "";

          return (
            <div
              onClick={() => onSelectItem(file.id)}
              className={`tab-item shrink-0 rounded-lg px-3 py-1.5 select-none text-gray-500 cursor-pointer hover:text-gray-400 flex items-center gap-2 ${
                active
              }`}
              key={item}
            >
      ...
      <div className="code-contents">
        {opened.map((item) => {
          ...
          return <CodeEditor key={item} id={selected} active={item === selected} language={getMonacoLanguage(dot[dot.length - 1])} />;
        })}
      </div>
    </div>
  );
